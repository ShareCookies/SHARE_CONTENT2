...
oo(面向对象)设计原则：
	...
	3.多用组合少用继承。p23
		...
		好处：
			...
			而继承仅是对代码的复用高，其可扩展性不如组合，使用组合不仅可以达到复用更可以在运行时动态的改变行为，后续的维护与扩展也更方便。
	4.开放-关闭原则：p86
		类应该对扩展开放，对修改关闭。
		即：
	...
	这个原则提供了一些方针，来降低类耦合:
		就任何对象而言,在该对象的方法内,我们只应该调用属于以下范围的方法:
			1.该对象本身
			2.被当做方法的参数而传递进来的对象
			3.此方法所创建或实例化的任何对象（该对象实例变量所引用的对象）
			4.对象的任何组件
			注：
				避免调用从另一个调用中返回的对象的方法。
				以上告诉我们，如果某对象调用其他方法返回的接口，不要调用该对象的方法。
				例：	
					不推荐：
	...
	注：
		...
		一个程序不可能遵守所有的原则或指导方针,应该在原则对我们有帮助的时候才遵守。
		所以应该尽量遵守原则,而不是随时都要遵循原则。			
附：

	便于记录： 嗯 还是写时 不断的回顾更好
	1
		类：单一职责原则，依赖倒置原则 针对接口编程
		属性：多用组合少用继承
		方法：
			开闭原则，封装变化 好莱坞原则
			松耦合 最少知识原则
	2
	父类(1.依赖倒置 2. 我调用你们 别调用我 3 4 封装变化、开放-关闭原则、) 祖父类
	子类(1. 针对接口编程 2. 多用组合少用继承 3 4. 高内聚 低耦合) 其余类
	最少盆友