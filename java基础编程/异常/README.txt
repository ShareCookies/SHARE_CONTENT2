...
异常的分类：
	java里异常分为Error和Exception

	Error：
		Error是程序中无法处理的错误，表示运行应用程序中出现了严重的错误。
		
		此类异常无法被捕获，通常表示代码运行时JVM出现的问题,所以发生此类错误时，JVM将终止对应线程。
		例：
			当jvm耗完可用内存时，将出现OutOfMemoryError等
			NoClassDefFoundError，比如说
		
	Exception：
		表程序可以处理的异常。
		该类型异常可分为两类。
		运行时异常：RuntimeException( unchecked exceptions不受检异常)
			都是RuntimeException类及其子类异常。
			表示程序在运行期间可能出现的错误。	
				例：用空值对象的引用（NullPointerException）、数组下标越界（ArrayIndexOutBoundException）。
			此类异常属于不可查异常，编译器不会检查此类异常。
			
			附：
				在程序中可以选择捕获处理，也可以不处理（包括可无需throw）。
		...
		非运行时异常：(受检异常)
			...
			附：
				如何处理受检异常：
						受检异常处理，除非后续程序均与抛出异常的地方，无关联，才可以捕获。
						不然建议抛出，否则很可能你捕获了，但后续程序需要对应的数据却获取不到，导致其他异常出现。
				为什么编译器会检查此类异常：
					因为受检异常跟程序运行的上下文环境有关，即使程序设计无误，运行期间仍然可能因使用的问题而引发。
					所以需要程序员进行处理。
		附：
			运行时异常与受检异常概念区别：
				运行时异常应是由用户使用不当造成的异常，所以程序可以无需处理，直接抛出提示用户。
				受检异常则与上下文环境有关(缺失必要条件造成)，所以程序可以决定捕获(捕获继续运行、捕获优化错误抛出提示用户)，或直接抛出。
